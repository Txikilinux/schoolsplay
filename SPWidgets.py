...
class RadioButton(Widget):
    ...
    def __init__(self, group, text, pos, fsize=18, padding=8, name='', **kwargs):
        ...
        self.name = name
        self.selected_image = utils.load_image(os.path.join(THEME['defaultpath'], 'radiobut_selected.png'))
        self.lbl = Label(text, pos, fsize)
        if not group:
            self._rb_group = {}
        else:
            self._rb_group = group._rb_group
        self._rb_selected = False
        self._rb_group[self] = self.lbl
        self._rb_default = False
    ...
    def set_as_default(self, default):
        self._rb_default = default
    
    def _rb_unselect(self):
        print "_unselect", self.name
        self.image.blit(self.box, (0, 0))
    
    def _cbf_on_select(self, *args):
        print "_cbf", self.name
        if self._rb_selected:
            ...
            for rb in self._rb_group:
                rb._rb_unselect()
                rb._rb_selected = False
            self.image.blit(self.selected_image, (1, 1))
...
class ImgTextButton(ImgButton):
    def __init__(self, path, text, pos=(0, 0), textpos=2, padding=4, fsize=24,\
        ...
        for line in text.split('\n'):
            if line == '':
                line = ' '
            surflist.append(utils.char2surf(line, fcol=fgcol, fsize=fsize, ttf=P_TTF))
    ...
    def __init__(self, path, hpath, text, pos=(0, 0), textpos=2, padding=4, fsize=24,\
        ...
        for line in text.split('\n'):
            if line == '':
                line = ' '
            surflist.append(utils.char2surf(line, fcol=fgcol, fsize=fsize, ttf=P_TTF))
            surfhlist.append(utils.char2surf(line, fcol=GREY, fsize=fsize, ttf=P_TTF))
        w = max([s.get_width() for s in surflist])
        h = surflist[0].get_height() 
        totalh = h * len(surflist)
        textsurf = pygame.Surface((w, totalh), SRCALPHA)
...
class ButtonDynamic(Button):
    def __init__(self, txt, pos=(0, 0), fsize=18, padding=4, name='', \
    ...
    def _setup(self, s):
        self._but, self._but_hover = make_button_bg_dynamic(self.r.width, \
                                                            self.sizename, \
                                                            self.colorname)
...
class Label(Widget):
    def __init__(self, txt, pos, fsize=18, padding=4, border=None, maxlen=0, name='', bold=False, ** kwargs):
...
class TextEntry(Widget):
    def __init__(self, pos, length=200, maxlen=0, message='', fsize = 28, border=None, \
        ...
        """
        self.logger = logging.getLogger("schoolsplay.SPWidgets_lgpl.TextEntry")
        self.length = length
        self.maxlen = maxlen
        self.fsize = fsize
        ...
        self.fgcol = fgcol
        if not self.fgcol:
            self.fgcol = THEME['textentry_fg_color']
        self.bgcol = bgcol
        if not self.bgcol:
            self.bgcol = THEME['textentry_bg_color']
        self.bold = bold
...
class TEB_TextEntry(TextEntry):
    ...
    def __init__(self, pos, length=200, maxlen=0, message='', fsize = 28, border=None, \
        ...
        self.logger = logging.getLogger("schoolsplay.SPWidgets_lgpl.TEB_TextEntry")
        self._TEfocus = False
...
class TextEntryBox(Widget):
    def __init__(self, pos, maxlen=0, length=300, height=2, message='', fsize = 14, border=True, \
        ...
        self.image.fill(THEME['textentry_bg_color'])
        Widget.__init__(self, self.image)
...
class Dialog(Widget):
    ...
    def __init__(self, txt, pos=None, fsize=12, butfsize=20, buttons=['OK'], title='', \
                 dialogwidth=300, dialogheight=0, name='', fbold=True):
...
#                       bgcol=THEME['dialog_but_bg_color'])
            b = ButtonDynamic(buttxt, (380,250), butfsize, padding=12, name=buttxt, fgcol=WHITE)
    ...
    def _setup(self):
        ...
        if type(self.txt) == types.ListType or type(self.txt) in types.StringTypes:
            if type(self.txt) == types.ListType:
                self.txt = '\n'.join(self.txt)
            tv = TextView(self.txt, (0, 0), pygame.Rect(0, 0, self.dialogwidth, 500),\
                           bgcol=THEME['dialog_bg_color'],fsize=self.fsize ,\
        ...
        self.action_area_rect = pygame.Rect(self.pos[0], action_area_y, action_area_x, y)
...
class DialogWindow(Dialog):
    ...
    Usage: TODO
    """
    def __init__(self, back, sprites, pos=None, butfsize=20,\
                  buttons=['OK'], title='', name=''):
        """screen - current pygame screen surface
        back - A SPSprite object with a image as large as needed to hold all the sprites.
        sprites - a list with all the sprites to display on the 'back' surface.
                The sprites must have the name attribute set.
        pos - is a tuple with x,y grid numbers. If not set the dialog is centered.
        buttons - a list with strings for the buttons (max two)
        name - to identify this object (optional) 
        """
        Dialog.__init__(self, back, pos=pos, fsize=12, butfsize=butfsize, buttons=buttons,\
                         title=title, dialogwidth=back.get_sprite_width(), dialogheight=0,\
                         name=name, fbold=True)
        self.show()
        x, y = self.get_action_area().topleft
        for s in sprites:
...
class PrevNextButton:
    ...
    def _setup(self):
        ...
        self.but.connect_callback(self._cbf, MOUSEBUTTONUP, 'prev')
    ...
    def toggle(self):
        ...
        if self.current is self.previmg:
            ...
            self.state = 'next'
            self.but.connect_callback(self._cbf, MOUSEBUTTONUP, 'next')
        else:
            self.current = self.previmg
...
class MenuBar:
    def __init__(self, rect, actives, cbf, dice_cbf,lock, usestar=False, \
        ...
        p = os.path.join(THEMESPATH, THEME['theme'],bname )
        hp = os.path.join(THEMESPATH, THEME['theme'],hbname )
        pos = (CORE_BUTTONS_XCOORDS[7], rect.top+8)
        if not os.path.exists(p):
            p = os.path.join(DEFAULTTHEMESPATH, bname)
        if not os.path.exists(hp):
            hp = None
        ...
        if usegraph:
            self.chartbutton = ChartButton((CORE_BUTTONS_XCOORDS[5], rect.top+8), cbf)
        ...
        self.scoredisplay = ScoreDisplay((CORE_BUTTONS_XCOORDS[6], rect.top+12), cbf=cbf)
...
class Graph(Widget):
    ...
    def __init__(self, data, level, headertext='', norm=None):
        ...
        self.logger = logging.getLogger("schoolsplay.SPWidgets_lgpl.Graph")
...
class VolumeAdjust:
    ...
    def __init__(self, pos, volume=None, defval=50):
        ...
        if volume or int(volume) == 0:
            ...
            #print "we have aumix"
            self.volume = int(WEHAVEAUMIX)
        else:
            #print "else defval"
            self.volume = int(defval)
        self.volstr = '%02d' % self.volume + "%"
        # TODO: set fgcol and bgcol kwargs ?
        self.lbl = Label(self.volstr, pos, fsize=18, padding=4, border=1, minh=48)
    ...
    def get_actives(self):
        return [self.voldownbut, self.volupbut, self.lbl, self.volumetoggle.get_actives()]
...
class ScrollWindow(Widget):
    ...
    def __init__(self, pos, size, objects, padding=4, childpadding=20, \
                 scrollstep=20, border=0, cols=0, button_up=None, button_up_ro=None, \
                 button_down=None, button_down_ro=None):
        """pos - position
        ...
        self.button_up_ro = button_up_ro
        self.button_down = button_down
        self.button_down_ro = button_down_ro
        pygame.display.update(pygame.draw.rect(self.screen, BLACK, self.rect, 2))
        ...
        for obj in objects:
            if firstobject:
                ...
                firstobject =False
            if x + obj.get_sprite_width() + childpadding > max_x or cols == 1:
                x = start_x + padding
                y = y + obj.get_sprite_height() + childpadding
            else:
                x += padding
    ...
    def _cbf(self, event, widget, data):
        if data[0] == 'down':
            if not self.rect.contains(self._lastchild.rect):
                if self.scrollstep < 0:
                    pass
                else:
                    self.scrollstep = -self.scrollstep
    ...
    def get_actives(self):
        return [self.upbut, self.downbut]
    ...
    def _scroll_children(self):
        ...
        for obj in self._objects:
            if obj._scroll_focus:# displayed object
                obj.erase_sprite()
                obj.rect.move_ip((0, self.scrollstep))
...
def make_button_bg_dynamic(width, sizename, colorname, colorname_ro='black'):
    ...
    x = width
    but = pygame.Surface((4+width + offset * 2, 4+hight), SRCALPHA)
    buthov = pygame.Surface((4+width + offset * 2, 4+hight), SRCALPHA)
...
if __name__ == '__main__':
    ...
    but = Button('Button', (10, 10), padding=6)
