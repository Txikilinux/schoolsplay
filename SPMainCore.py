...
class MainCoreGui:
    def __init__(self, resolution=(800, 600), options=None, \
        ...
        if self.COPmode:
            ...
            theme_dir = os.path.join(ACTIVITYDATADIR, 'SPData', 'themes', self.theme)
            xmlname = 'SP_menu.xml'
    ...
    def start_next_level(self, store_db=None, nextlevel=None, levelup=False):
        ...
        self.logger.debug("start_next_level called with: %s, %s, %s" % (store_db,nextlevel,levelup))
        self.clear_screen()
        self.screen.set_clip()
        if nextlevel < 1:
            nextlevel = 1
    ...
    def activity_level_end(self, store_db=None, nextlevel=1, levelup=False):
        """The level is ended.
        The core will call next_level on the activity.
        ...
        if not self.DTactivity:
            if not sc or sc < 7.0:
                ...
                if levelup and nextlevel > 1:
                    ...
                    nextlevel -= 1
                    levelup = False
        self.logger.debug("activity_level_end final values:  %s,%s,%s" % \
                          (store_db, nextlevel, levelup))
